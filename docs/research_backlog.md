...
| **Iraq / Saudi Arabia / Iran gas: fresh standalone GulfPub recon workbooks (2026-07-29)** | Produced by the length-units re-run (§4) and **not in any packet**. Iraq (`--commodity both`, 47 refs): gas 19 overlaps / 7 additions / 40 GEM-only / 5 status conflicts / 9 ambiguous + oil 20/1/65/8/5. Saudi (20 refs): 18 overlaps / 2 additions / 23 GEM-only / **9 status conflicts** / 1 ambiguous. Iran (43 refs): 25 overlaps / 18 additions / 25 GEM-only / 3 status conflicts / 10 ambiguous. **Saudi's and Iran's pre-fix runs date from 07-05/07-06 and predate the admin-area geo signal, per-dataset matching overrides and the 'very low' re-grade — read these as fresh runs, not deltas**, and expect their packets' `Gas_GulfPub` crosswalk tabs to be stale | `batches/{iraq-gas,saudi-arabia-gas,iran-gas}/deliverables/pipelines_batch_20260729_0941_ET_*_reconciliation-gulfpub.xlsx` |
